...
20a. `skillsets/ux-design-intelligence/shared/ui-ux-pro-max/SKILL.md` when UX planning or implementation needs focused local research on patterns, accessibility, typography, color, charts, icons, or framework practices; keep the active product brand and design system authoritative
...
- When local-sidecar delegation is required, make multiple independent
  ...
  final judgment.
- In Codex, route bounded quick or standard tool/file work to the fastest
  capable model discovered in the live catalog when validation is cheap.
  Before using a stronger tier, record why the fast lane is insufficient.
