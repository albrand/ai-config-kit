...
| `skillsets/orca-workflow-automation/` | Explicit-only Codex skill for Orca-owned workflow automation. Orca owns schedule/worktree/terminal/browser-emulator lifecycle. Two stdlib helpers: read-only PR-review queue (`orca-pr-review-queue.py`; private/draft-only, exact head-SHA dedup, self-review prevention, no auto-merge) and privacy-safe execution ledger (`execution-ledger.py`; allowlist-only JSONL, never records prompts/transcripts/env/secrets/repo/branch/SHA, aggregates only after a min sample count). Offline unit tests included. |
