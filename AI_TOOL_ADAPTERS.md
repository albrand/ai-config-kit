...
- For portable, capability-first discovery of the host software an agent runs in
  ...
  shells/harnesses), install the complete
  `skillsets/native-agent-surfaces/codex/native-agent-surface/` skill and follow
  `NATIVE_AGENT_SURFACES.md`. Keep the skill explicit-only. Run the detector to
...
- If the adopted profile requires a sidecar, run at least one independent
...
- If Codex model routing is available, discover the live catalog and use its
...
If Codex model routing is available, state which first safe bounded tool/file
sidecar the fastest capable live model should handle and why a stronger tier is
needed when the fast lane is skipped.
