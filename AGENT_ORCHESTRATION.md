...
- In Codex, the fastest capable model verified in the live catalog can handle
  the first safe sidecar for quick or standard work while the master keeps
  architecture, integration, and final validation.
...
- For Codex model routing, default bounded low-risk tool/file execution to the
  fastest capable model verified in the live catalog. Before using a stronger
  tier, record the exception reason.
