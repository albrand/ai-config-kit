...
| `high-signal-pr-review` | Reviewing PRs where false positives are costly | Preflight stop checks, independent issue discovery, validation pass, deduped findings, default submitted PR reviews, inline threads when possible |
| `ux-design-agent` | Design-maker work: live mockups/prototypes, layouts, design tokens, typography, components, design systems, a design preview/deploy for review, design signoff, or handoff to Figma and a ticket board | Design-maker detection, source-of-truth enforcement, capability gate, UX questions with defaults, live mockup + preview URL, design signoff, propagation to Figma (or chosen SoT) and to Linear/Jira tickets with a revision comment, validation |
...
| `skillsets/ux-design-agent/` | A design-maker (UX designer, founder, or product team) wants a personal AI design agent that designs in a live mockup (e.g. Next.js on Vercel), enforces one source of truth, runs a design signoff, and propagates signed-off UI to Figma and a ticket board (Linear/Jira), plus layouts, design tokens, typography, system conventions, component-library guidance such as shadcn/ui, Figma annotations, or design-to-code handoff | Design-maker detection, source-of-truth gate, mode detection, Figma/board/Vercel capability gate, UX questions with recommended defaults, live mockup + preview URL, token and design-system decision, design signoff, propagation to Figma (or chosen SoT) and to tickets with a revision comment, artifacts changed or proposed, UX validation |
...
| `skillsets/pr-review/` | A user wants `/code-review`, high-signal PR review, diff review, merge readiness, public review comments, or AI-generated PR review | Preflight decision, scoped instruction map, validated findings, dropped candidates, posting mode, validation reviewed, and residual risk |
