...
| `native-agent-surface` | An agent needs to discover which native host surfaces are present (cmux, tmux, zellij, generic agentic shells/harnesses), route on declared capabilities instead of hard-coding one tool, safely target discovered surfaces, or decide reuse-vs-create for a workspace | Capability gate, JSON surface report (binaries + adapter status + candidate runtime capabilities + declared capabilities), host-neutral adapter routing, reuse-first workspace resolver (`reuse|missing|ambiguous|unsupported`; exact wins, inside-project eligible, broad-parent advisory, ties fail closed), full-identifier validation, fail-closed on missing surface/identifier/denied env value, never-serialize-env-values guarantee |
...
| `skillsets/native-agent-surfaces/` plus `NATIVE_AGENT_SURFACES.md` | An agent or operator wants portable, capability-first discovery of the host software an agent runs in or through (cmux first adapter; also tmux, zellij, and generic agentic shells/harnesses) without hard-coding one tool or serializing env values, or wants to reuse a workspace before creating a duplicate | Capability gate, host-neutral detector JSON report, declared-capability routing, adapter status + candidate runtime capabilities, reuse-first workspace resolver, adapter contract, project-setup / browser-E2E / session-coordination references, full-identifier validation, hard never-serialize-`CMUX_*`/socket-capability boundary, offline self-tests |
