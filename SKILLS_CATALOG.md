...
| `opencode-fast-execution` | Bounded OpenCode/GLM execution, premature-stop diagnosis, managed same-step repair, or safe context cleanup | Compact packet, evidence-based continue/stop decision, exact stop reason, process-quiescence evidence, run telemetry, and safe session-retention result |
...
| --- | --- | --- |
| `skillsets/adaptive-model-orchestration/` | A user or repo adopts adaptive or always-on two-family routing, OpenCode/GLM sidecars, Codex fast/balanced/deep peers, needs an xhigh versus max/ultra decision, or diagnoses sidecar liveness | Capability and authorization gate, routing record, bounded briefs, evidence-based stop authority, process-quiescence status, lane status, locally revalidated conclusions, and residual risk |
