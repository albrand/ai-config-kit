...
| `execution-ownership` | Before expensive, release, migration, or otherwise single-flight agent work that must not run concurrently for the same repository, worktree, branch, commit, and operation | Host-local atomic lease with owner metadata, heartbeat/renewal, release, deterministic held/stale/error states, stale-owner reclaim rules, and fail-closed path/permission/symlink handling |
