...
| `cmux-hermes-orchestrator` | An operator has adopted cmux as the local UI/session surface and Hermes as the remote provider router (Tailscale-only VPS) and wants bounded, default-off, one-writer-per-task orchestration | Capability gate, token-free doctor, write-isolated lane/worktree with a non-focused cmux workspace, fail-closed noninteractive advisor, persistent Hermes master in remote tmux, exact per-session usage, preserving cancel/close, report-only cleanup |
| `native-agent-surface` | An agent needs to discover which native host surfaces are present (cmux, tmux, zellij, generic agentic shells/harnesses), route on declared capabilities instead of hard-coding one tool, or safely target discovered surfaces | Capability gate, JSON surface report (binaries + safe metadata + declared capabilities), host-neutral adapter routing, full-identifier validation, fail-closed on missing surface/identifier/denied env value, never-serialize-env-values guarantee |
...
| `skillsets/native-agent-surfaces/` plus `NATIVE_AGENT_SURFACES.md` | An agent or operator wants portable, capability-first discovery of the host software an agent runs in or through (cmux first adapter; also tmux, zellij, and generic agentic shells/harnesses) without hard-coding one tool or serializing env values | Capability gate, host-neutral detector JSON report, declared-capability routing, adapter contract, full-identifier validation, hard never-serialize-`CMUX_*`/socket-capability boundary, offline self-test |
