...
if (args[0] === 'session' && args[1] === 'list') {
  if (process.env.FAKE_EMPTY_SESSION_LIST === '1') process.exit(0);
...
try {
  const leaseDir = path.join(temp, 'context', 'leases');
  fs.mkdirSync(leaseDir, { recursive: true });
  const staleLeasePath = path.join(leaseDir, 'stale.json');
  fs.writeFileSync(
    staleLeasePath,
    JSON.stringify({
      run_id: 'stale-run',
  ...
  const defaultDeadline = runManaged([], {
    FAKE_MODE: 'slow-success',
    BB_THREAD_ID: 'thread-lease-test',
    BB_ENVIRONMENT_ID: 'environment-lease-test',
  });
  assert.equal(defaultDeadline.status, 0, defaultDeadline.stderr);
  assert.equal(readTelemetry(defaultDeadline).stop_reason, 'completed');
  assert.equal(readTelemetry(defaultDeadline).explicit_deadline_ms, 1_800_000);
  assert.equal(readTelemetry(defaultDeadline).deleted, true);
  assert.equal(JSON.parse(fs.readFileSync(staleLeasePath, 'utf8')).state, 'stale');
  const defaultLease = JSON.parse(
    fs.readFileSync(readTelemetry(defaultDeadline).lease, 'utf8'),
  );
  assert.equal(defaultLease.thread_id, 'thread-lease-test');
  assert.equal(defaultLease.environment_id, 'environment-lease-test');

  fs.rmSync(statePath, { force: true });
  const emptySessionListing = runManaged([], { FAKE_EMPTY_SESSION_LIST: '1' });
  assert.equal(emptySessionListing.status, 0, emptySessionListing.stderr);
  assert.equal(readTelemetry(emptySessionListing).session_ownership_reliable, false);
  assert.equal(readTelemetry(emptySessionListing).newly_owned, false);
  assert.equal(readTelemetry(emptySessionListing).deleted, false);
