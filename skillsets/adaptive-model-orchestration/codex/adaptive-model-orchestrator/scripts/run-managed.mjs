...
const DEFAULT_MANAGED_DEADLINE_MS = 30 * 60 * 1_000;
...
const requestedDeadlineMs = readPositiveNumber('--deadline-ms');
const deadlineMs = requestedDeadlineMs ?? DEFAULT_MANAGED_DEADLINE_MS;
...
function listSessions() {
  // OpenCode 1.17.12 truncates piped session-list output at 64 KiB. The
  // newest 150 sessions stay below that boundary with headroom, and a session created by
  // this run is necessarily in that newest window for ownership comparison.
  const result = spawnSync(command, ['session', 'list', '--format', 'json', '-n', '150'], {
    encoding: 'utf8',
    maxBuffer: 4_194_304,
  });
  ...
  // OpenCode 1.17.12 can return an empty successful response while its local
  // session index is being rebuilt. Permit the new run, but never infer that a
  // session is newly owned from that incomplete listing or delete it later.
  if (output.length === 0) {
    return { ownershipReliable: false, sessions: [] };
  }
  ...
  try {
    ...
    throw new Error(
      `OpenCode session list returned invalid JSON: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  return { ownershipReliable: true, sessions };
}
...
const existingSessionListing = listSessions();
const existingSessions = existingSessionListing.sessions;
const existingIds = new Set(existingSessions.map((session) => session.id));
const runId = crypto.randomUUID();
const contextRoot = path.resolve(
  process.env.OPENCODE_CONTEXT_DIR ??
    (process.env.BB_DATA_DIR
      ? path.join(process.env.BB_DATA_DIR, 'opencode-context')
      : path.join(os.tmpdir(), 'opencode-context')),
);
...
function writeLeaseAt(targetPath, value) {
...
function reconcileStaleLeases() {
  let entries;

  try {
    entries = fs.readdirSync(leaseDir, { withFileTypes: true });
  } catch {
    return;
  }
  ...
  for (const entry of entries) {
    ...
    const candidatePath = path.join(leaseDir, entry.name);
    let candidate;
    ...
    if (candidate?.state !== 'running' || candidate.run_id === runId) continue;

    if (isPidAlive(Number(candidate.caller_pid)) || isPidAlive(Number(candidate.child_pid))) {
      continue;
    }

    try {
      writeLeaseAt(candidatePath, {
        ...candidate,
        ended_at: new Date().toISOString(),
        state: 'stale',
        stop_reason: 'owner_and_child_absent',
        process_quiescent: true,
      });
    } catch {
      // A concurrent runner may have finalized this lease; leave it intact.
    }
  }
}
...
writeLease({
  ...
  child_pid: child.pid,
  thread_id: process.env.BB_THREAD_ID ?? null,
  environment_id: process.env.BB_ENVIRONMENT_ID ?? null,
  provider_id: process.env.BB_PROVIDER_ID ?? process.env.BB_PROVIDER ?? null,
  cwd: process.cwd(),
  command,
  session_id: null,
...
async function finalize(code, childSignal) {
  ...
  const newlyOwned = Boolean(
    existingSessionListing.ownershipReliable && sessionId && !existingIds.has(sessionId),
  );
  ...
  const telemetry = {
    context_gc: {
      ...
      session_ownership_reliable: existingSessionListing.ownershipReliable,
