...
unchanged. The runner applies a bounded 30-minute outer deadline by default. An
operator may declare a shorter or longer explicit deadline with
`OPENCODE_RUN_DEADLINE_MS`; an undeclared idle limit must not be invented during
a run. Same-step continuation requires
