...
- If the adopted profile requires a sidecar, run at least one independent
  bounded lane for substantive work. Add multiple blind passes only when risk
  or independent questions justify them, then reconcile the outputs.
- In Codex, prefer the fastest capable model verified in the live catalog for
  the first safe bounded tool/file sidecar when validation is cheap. Before
  using a stronger tier, record why the fast lane is insufficient.
