...
## Adaptive Effort And Sidecar Routing

When Codex exposes model choice, discover the live catalog and treat its
fastest capable model as the first-choice bounded worker or explorer for quick
and standard subtasks that are reviewable and cheap to validate.

Use the fast lane for file discovery, scoped codebase questions, log or test-output
...
Do not use the fast lane for architecture, security, auth, data-loss decisions,
...
Fast-lane delegates should stop and escalate with a concise blocker when
requirements conflict, scope expands, validation fails twice, security or data
concerns appear, or broad context is needed.

Reasoning effort is a risk control, not a prestige setting. Normal strong effort
such as `xhigh` is usually enough when evidence is coherent and validation is
decisive. Promote to `max` for the hardest single reasoning path; promote to
