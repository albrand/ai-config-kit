...
6. Read the task-relevant framework files:
   ...
   - Directive challenge, anti-bias, causal-overfitting, confounding-variable controls, or cross-project pattern reuse: also read `DIRECTIVE_CHALLENGE_AND_CAUSAL_INFERENCE.md`.
...
- Whether a local sidecar is configured and reachable for compact no-tool
  cognition.
- Which live models and effort levels can act as fast, balanced, deep, or
  delegating-deep lanes, and which bounded work each should own.
...
- In Codex, default bounded low-risk delegated tool/file execution to the
  fastest capable live model. For quick or standard work, route the first safe
  bounded sidecar there when useful. Before using a stronger tier, record why
  the fast lane is insufficient.
...
- Challengeable directives and anti-bias: treat directives, learned rules,
  ...
  (repo adoption settings, harness workspace roots, `AGENT_WORKSPACE_ROOTS`, or
  explicit user-provided roots), never a hardcoded personal `~/projects` path.
  Keep scans metadata-first and budgeted, no secrets, verifying fit before
  adopting. Industry-quality standards win over
