...
## Adaptive Routing Order

Apply this order before choosing a frontier path:

1. **Fast capable lane first** for bounded cognition:
...
2. **Balanced lane second** for bounded worker or explorer tasks that need
   file, shell, repo, or validation access and can be checked by the master.
...
The sidecar is not a source-of-truth agent. Its tool access is mode-specific and
must not receive secrets or large private context. Use short prompts, hard
output caps, tool-free advisor instructions, and a timeout. If it is
unavailable, slow, or inconclusive, use another verified bounded worker or keep
the work in the master thread.

## Local Sidecar Delegation Gate

When a configured sidecar is reachable and the adopted profile requires it,
the coordinator must run at least one independent bounded lane for substantive
work before implementation or final judgment.

Choose only roles that can materially change the result:
...
Do not run all roles mechanically. Use multiple blind passes for high-risk
reviews or genuinely independent questions; otherwise one well-bounded lane is
enough. Follow `ADAPTIVE_MODEL_ORCHESTRATION.md` for profile and effort choice.
...
## Fast Bounded Worker Profile

When the active harness exposes model choice, discover the live catalog and use
the fastest capable model as the default bounded execution tier for low-risk
work. Do not hard-code a model slug into portable doctrine.

For quick or standard workflows, route the first safe bounded sidecar to it
...
Before routing a delegated quick or standard subtask to a stronger tier,
run a fast-lane fit check and record the exception reason when it is not used.
...
Fast-lane delegates must stop and return a concise blocker when scope expands,
...
## Model Family And Effort Routing

Routing is tier-first and family-aware. Assign roles from verified capability,
not provider habit or a stale model name.

- **Active harness model (coordinator).** Owns intent, architecture, integration,
  escalation, and final validation truth regardless of provider.
- **Fast or balanced peer.** Owns bounded discovery, tool execution, test/log
  triage, mechanical verification, or an independent plan/code critique.
