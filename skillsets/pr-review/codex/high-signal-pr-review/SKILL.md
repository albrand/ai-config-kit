...
4. Read intent before details: PR title, body, linked issue, changed-file list, and author-stated tradeoffs.
5. Decide whether independent review passes are useful and available. If the platform permits sub-agents and runtime policy allows it, use bounded passes for instruction adherence, bug/security/logic, validation/test coverage, and architecture. The master thread filters and owns the final verdict.
6. Review changed code with surrounding context only as needed. Prioritize:
...
7. Apply Matt-inspired engineering checks where relevant:
...
8. Validate every candidate finding before reporting. Drop speculative, lint-only, style-only, pre-existing, unscoped, or unsupported issues.
9. If comment mode was not requested, report findings only. If comment mode was requested, prepare a private comment plan, dedupe findings, and post only approved high-confidence comments.
10. Produce the final report using the output contract.
