...
11. For GitHub PRs, prepare a private comment plan, dedupe findings, run the contract's pre-post freshness check and pre-post self-check, and post only approved high-confidence review comments by default. Prefer one submitted PR review over loose issue comments.
    ...
    - Each substantive thread must explain: the business rule or contract being violated (the source rule/acceptance criterion), the root cause in the changed code (changed-code causality), what the code does now, a practical failure example, validation or falsification (how the claim was checked and what would refute it), severity and confidence with a one-line basis, the negative impact of keeping the change as-is, and a concrete next step for the developer (the fix direction). Post each finding on the smallest owning changed range. Drop unsupported, speculative, or merely pre-existing findings before posting.
