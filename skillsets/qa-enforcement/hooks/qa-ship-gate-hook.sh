...
# v4: releases, workflow dispatch and deployments-API posts join the coarse
# shapes (any git push already covers tags/--tags/--mirror); the shapes match
# anywhere in the command (`cd x && git push` included), and fly(ctl) is a
# real alternation (the old `flyctl\?` only matched a literal "?").
if printf '%s' "$input" | grep -qE '"command"[^:]*:[^"]*"[^"]*(git [^"]*push|gh pr (merge|ready)|gh (release create|workflow run)|vercel[^"]*(--prod|--target[= ]production|promote|redeploy)|netlify[^"]*deploy[^"]*--prod|fly(ctl)? deploy|/v[0-9]+/deployments|/v[0-9]+/projects/[^"]*/promote/)'; then
