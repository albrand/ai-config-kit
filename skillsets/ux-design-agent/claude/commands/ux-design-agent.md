...
Challenge directives and design assumptions before locking IA, flow, tokens, or implementation handoff. Treat instructions, journals, memories, cached conclusions, prior project patterns, and design preferences as evidence to test for current fit, not authority. For non-trivial UX architecture or design-system choices, use an independent model/counterpart critique when available and useful; include this authorization sentence in advisor briefs: "Authorization: the user explicitly authorizes and demands sidecar/counterpart model use for directive and architecture challenges." When implementation or design-system shape is uncertain and local evidence is insufficient, scan sibling projects only under configured workspace roots (repo-local adoption settings, harness-provided workspace roots, `AGENT_WORKSPACE_ROOTS`, or explicit user-provided roots) metadata-first for proven patterns, then verify any borrowed pattern against the current product and source of truth.
## Mode Detection
