...
verify: 'python3 "$HOME/.agents/skills/ui-ux-pro-max/scripts/verify.py"'
verified: 2026-09-03
---
...
This skill complements `ux-design-agent`: that skill owns product workflow,
source-of-truth decisions, signoff, and handoff; this one supplies focused local
retrieval and implementation checklists. When both apply, run the workflow from
`ux-design-agent` and use this search tool only for the specific design questions
that arise.
...
For an existing product, preserve its verified brand tokens, components, and
interaction conventions. Do not replace them because `--design-system` returned a
different palette or typeface. Treat every result as a candidate, verify product
fit and source freshness, and prefer focused domain/stack searches over a broad
visual reset. Never persist a generated design system or overwrite an existing
...
The search script lives inside this skill's own directory, not the project directory. Resolve `<skill-root>` as the directory containing this `SKILL.md`, then invoke the script by its full path — do not assume a particular working directory:

```bash
python3 "<skill-root>/scripts/search.py" "<query>" --domain <domain>
```

If `python3` is not found, try `python`, then `py -3`. Requires Python 3.x and no external dependencies. Do not install system packages on the user's behalf.
...
1. **New product or explicitly authorized system-wide visual direction** → use `--design-system`.
   For a new page in an established product, inherit the authoritative system and
   use focused domain/stack searches instead.
...
### Step 2: Generate Design System (new products or authorized system work)
...
# Step 3: supplement
python3 "<skill-root>/scripts/search.py" "keyboard focus modal" --domain ux
...
- Use `--design-system` for a new product or authorized system direction; use
  focused domain/stack searches for a new page in an established product
